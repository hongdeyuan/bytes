...
var numberIsFinite = function (v) { return isFinite(v); };
...
function format(value, options) {
    ...
    if (toUnit) {
        ...
        var matchUnit = keys.find(function (k) {
            return map[k] &&
                mag >= map[k] &&
                Math.round(mag / map[k]) === mag / map[k];
        });
        if (matchUnit) {
        ...
        var matchUnit = keys.find(function (k) { return map[k] && mag >= map[k]; });
        if (matchUnit) {
            unit = matchUnit.toUpperCase();
        }
    }
...
function parseToUnit(value, toUnit, capacityBase) {
    ...
    return typeof parseVal === 'number' ? format(parseVal, { capacityBase: capacityBase, toUnit: toUnit }) : null;
}
var bytes = {
    format: format,
    parse: parse,
